...
4. 렌더링 트리에서 레이아웃을 실행하여 **각 엘리먼트의 기하학적 형태(넓이와 높이)와 위치를 계산 (Reflow)**합니다.
...
BODY, P, DIV, IMG 등의 각 엘리먼트의 위치와 크기, 바탕색, 선 굵기 등을 알아야 각자의 위치에 알맞게 아름다운 화면을 만들 수 있습니다.

이 구실을 하는 것이 <strong><u>렌더링 트리</u></strong>입니다. 렌더링 트리는 DOM 트리와 CSSOM 트리가 결합해서 만들어집니다.

렌더링 트리는 화면에 표시되는 각 엘리먼트의 위치를 계산하고, 각자의 위치에  화면에 색칠을 하는 등등의 구실을 합니다. <u>렌더링 트리가 만들어지면 브라우저는 엘리먼트를 화면에 표시(페인트)</u>할 수 있습니다.
...
렌더링 트리는 화면에 페인트하기 위해서 각 엘리먼트의 기하학적 구조(넓이와 높이)와 위치를 계산합니다. 
엘리먼트의 테두리 두께를 바꾸거나, 문단에 텍스트를 추가해서 줄이 늘어나는 등의 변화가 생기면 엘리먼트의 기하학적 구조와 위치도 다시 계산해야 합니다.
이를 리플로우라고 합니다. 
...
{: .box-note}
1) 엘리먼트를 숨기고 변경한 후 다시 드러냅니다.(최소 리플로우가 두 번 발생합니다.)

{: .box-note}
2) 현재 DOM 바깥에서 문서 조각을 만들어 변경한 후 문서에 복사합니다.(최소 리플로우가 한 번 발생합니다.)

{: .box-note}
3) 현재 엘리먼트를 문서 밖의 노드에 복사해서 사본을 변경한 후 원래 엘리먼트를 대체합니다.(최소 리플로우가 두 번 발생합니다.)
...
display = none 상태에서 엘리먼트에 대한 수정 작업을 모두 마친 후, 다시 display속성을 block이나 inline 등으로 변경하면 리플로우를 최소 두 번 정도로 줄일 수 있습니다. <br>
