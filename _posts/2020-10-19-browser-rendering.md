...
4. 렌더링 트리에서 레이아웃을 실행하여 **각 노드의 기하학적 형태(넓이와 높이)와 위치를 계산 (Reflow)**합니다.
...
여러 요소가 맞물려 있는데, 그 중 하나만 크기가 커지면 틀이 망가지게 됩니다. 이때 그 한 개를 제외한 나머지 모든 퍼즐의 높이와 넓이, 위치를 모두 다시 계산해서 재배치 해야 합니다. 

이 재배치를 위해 기하학적 구조와 위치를 다시 계산하는 것을 Replow라고 이해하면 됩니다. <br>
...
크롬 개발자 도구의 Performance 툴을 아주 잘 사용하는 사람들은 잘 알겠지만, Repaint보다는 리플로우가 더 빈번한 문제를 낳습니다. 
...
{: .box-note}
1) 요소를 숨기고 변경한 후 다시 드러냅니다.(최소 리플로우가 두 번 발생합니다.)

{: .box-note}
2) 현재 DOM 바깥에서 문서 조각을 만들어 변경한 후 문서에 복사합니다.(최소 리플로우가 한 번 발생합니다.)

{: .box-note}
3) 현재 요소를 문서 밖의 노드에 복사해서 사본을 변경한 후 원래 요소를 대체합니다.(최소 리플로우가 두 번 발생합니다.)


#### 가) 요소를 숨기고 변경한 후 다시 드러내는 방법
...
display = none 상태에서 요소에 대한 수정 작업을 모두 마친 후, 다시 display속성을 block이나 inline 등으로 변경하면 리플로우를 최소 두 번 정도로 줄일 수 있습니다. <br>
(display = none 으로 변경할 때, display = block을 변경할 때) 
...
createDocumentFragment는 DOM 노드들입니다. 하지만, 메인 DOM 트리에는 속하지 않습니다. <br>
...
```
<br>
#### 다) 현재 요소를 문서 밖의 노드에 복사해서 사본을 변경한 후 원래 요소를 대체하는 방법

마지막 방법은 수정하려는 노드를 복사해서 사본을 수정한 후, 원래 노드를 사본으로 대체하는 방법입니다. 
...
Angular와 Ember는 React와 Vue 등과는 달리 html을 별도의 템플릿으로 관리합니다. 같은 컴포넌트에 html 요소를 같이 렌더링하는 React 와 Vue 등의 프레임워크(라이브러리)와 달리 Viutaul DOM을 도입하기 어렵습니다. 그 대신, Angular는 9 버전부터 Incremental DOM(증가형 돔)이라는 것을 도입했습니다. 
