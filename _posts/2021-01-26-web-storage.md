...
미리 정의된 데이터 열과 행으로 이루어진 테이블 기반의 데이터베이스와는 달리 <u>Javascript 기반의 객체지향 데이터베이스</u>입니다. <br>
데이터베이스 안에 여러 개의 객체 저장소가 있고, 그 객체 저장소에는 키(key)-값(value) 쌍으로 된 레코드가 들어있는 식입니다. 

명칭에서도 알 수 있듯이 인덱스를 사용해 객체 저장소를 쿼리할 수 있고, 쿼리와 매칭되는 레코드만 순회하여 탐색할 수 있습니다.<br>
